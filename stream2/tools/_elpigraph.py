...
def learn_graph(
    ...
    n_jobs=None,
    GPU=False,
    max_candidates={"AddNode2Node": 20, "BisectEdge": 20, "ShrinkEdge": 50},
    store_evolution=False,
    **kwargs,
):
    ...
    use_partition: bool
        Whether to learn a disconnected graph
        for each category in adata.uns['partition']
    use_weights: bool
        Whether to weight points with adata.obs['pointweights']
    GPU:
        Whether to perform computations using GPU (requires cupy library)
    max_candidates:
        Max number of candidates to generate with each graph grammar
        when exploring graph topology at each iteration.
        Setting numbers lower can increase speed,
        especially for higher number of nodes.
    store_evolution:
        Store the evolution of the graph for each number of nodes
    **kwargs:
    ...
    if use_partition:
        ...
        if store_evolution:
            raise ValueError(
                "can't use store_evolution=True when use_partition=True"
            )
        ...
        for part in adata.obs["partition"].unique():
            ...
            if part not in partitions:
                ...
                _learn_graph(
                    ...
                    epg_mu=epg_mu,
                    epg_alpha=epg_alpha,
                    epg_trimming_radius=epg_trimming_radius,
                    use_seed=use_seed,
                    ...
                    store_evolution=store_evolution,
                    **kwargs,
                )
...
def _learn_graph(
    ...
    **kwargs:
    ...
    if ordinal_label is not None:
        if type(ordinal_label) == str:
            kwargs["pseudotime"] = adata.obs[ordinal_label].to_numpy()
        else:
            raise ValueError(f"ordinal_label key not found in adata.obs.")
    ...
    if method == "principal_curve":
        dict_epg = elpigraph.computeElasticPrincipalCurve(
            X=mat,
            NumNodes=n_nodes,
            TrimmingRadius=epg_trimming_radius,
            n_cores=n_jobs,
            Do_PCA=False,
            CenterData=False,
            Lambda=epg_lambda,
            ...
            GPU=GPU,
            MaxNumberOfGraphCandidatesDict=max_candidates,
            StoreGraphEvolution=store_evolution,
            **kwargs,
        )[0]
...
def seed_graph(
    ...
    force: bool
        ...
        Force supervised MST initialization to follow
        specified paths rather than using soft constraint
    use_weights: bool
        Whether to weight points with adata.obs['pointweights']
    use_partition: bool
        Whether to learn a disconnected graph
    ...
    Returns
    -------
    adata.obs['clustering']: `pandas.core.series.Series`
...
def _seed_graph(
    ...
    if clustering == "ap":
        ...
        if use_weights:
            ...
            weights = np.array(adata.obs["pointweights"]).flatten()
        else:
            weights = None
        kmeans = KMeans(
