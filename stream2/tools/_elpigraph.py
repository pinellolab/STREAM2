...
def learn_graph(
    ...
    **kwargs,
    ...
    use_partition: bool
        Whether to learn a disconnected graph for each category in adata.uns['partition']
    use_weights: bool
        Whether to weight points with adata.obs['pointweights']
    GPU:
        Whether to perform computations using GPU (requires the cupy library)
    max_candidates:
        Max number of candidates to generate with each graph grammar
        when exploring graph topology at each iteration. Setting numbers lower
        can greatly increase speed, especially for higher number of nodes.
    store_evolution:
