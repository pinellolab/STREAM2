...
@pytest.fixture
def adata():
    return st.read_h5ad("tests/data/rnaseq_paul15.h5ad")


def test_stream2_rnaseq_paul15(adata, tmp_path):
    st.settings.set_workdir(tmp_path / "result_rnaseq_paul15")
    st.settings.set_figure_params(
        dpi=80, style="white", fig_size=[5, 5], rc={"image.cmap": "viridis"}
    )
    ...
    st.tl.infer_pseudotime(adata, source=0, target=4)
    st.pl.graph(
        adata, color=["epg_pseudotime"], show_text=False, show_node=False
    )
